...
def _create_session(model_path: Path, *, use_vulkan: bool = True) -> ort.InferenceSession:
    ...
    # Disable arena pool so ONNX releases memory back to the OS immediately
...
def _embed(session: ort.InferenceSession, tokenizer: Tokenizer, texts: list[str]) -> np.ndarray:
    ...
    del inputs, outputs, last_hidden, mask, mask_expanded, summed, counts, embeddings, norms
    return result
...
class PrototypeClassifier:
    ...
    def select_workflow(
        ...
        """Select the most semantically similar available workflow."""
        labels = [
            f"{candidate['name'].replace('-', ' ').replace('_', ' ')} "
            f"{candidate['description']} for {candidate['voice']}"
            for candidate in candidates
        ]
        embeddings = _embed(self.session, self.tokenizer, [text, *labels])
        scores = embeddings[1:] @ embeddings[0]
        selected = int(np.argmax(scores))
        confidence = float((scores[selected] + 1.0) / 2.0)
        candidate = candidates[selected]
        return candidate["name"], candidate["voice"], confidence
...
async def chat_completions(request: Request) -> JSONResponse:
    ...
    payload_data: dict[str, str | float | None] = {
        "category": category,
        "confidence": round(confidence, 4),
    }
    workflows = body.get("workflows", [])
    if isinstance(workflows, list) and workflows and all(
        ...
        workflow, voice, workflow_confidence = clf.select_workflow(user_text, workflows)
        payload_data.update(
            workflow=workflow,
            voice=voice,
