...
| **TODO List Executor** | Detects when work items need tracking. Creates or updates the single per-voice file `config/kateto/voices/{voice}/TODO.md` with Markdown checkbox tasks (`- [ ]` / `- [x]`). It does not create project subdirectories or global TODO files; the backlog remains canonical, and completion is synchronized via MCP. |
